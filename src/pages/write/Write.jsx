...
const Write = () => {
    ...
    if (postId) {
      try {
        ...
        const getPostToUpdate = async () => {
          const postToUpdate = await axios.get(
            `https://1001devs.arabickitchenis.life/api/posts/read/${postId}`
          );
  ...
  const publish = (postBody) => {
    ...
    if (isLoading) {
      setIsFetching(true);
      axios
        .post('https://1001devs.arabickitchenis.life/api/posts/write', {
      ...
      axios
        .put(`https://1001devs.arabickitchenis.life/api/posts/${postId}`, {
          username: user.username,
